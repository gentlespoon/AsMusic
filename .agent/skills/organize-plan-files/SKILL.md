...
4. Without checking the file content, categorize the file by its feature. Move the files to feature subdirectory. For example, if a file plans for playlist related functionalities, move it to `playlist` sub-directory. Create new category if needed, do not forcefully fit something in an unrelated category.
