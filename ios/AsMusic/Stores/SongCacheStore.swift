...
actor SongCacheStore {
  ...
  func loadSongs(for cacheKey: String) -> [Song]? {
    guard openIfNeeded() else { return nil }

    let sql = "SELECT payload FROM song_cache WHERE cache_key = ? LIMIT 1;"
  ...
  func saveSongs(_ songs: [Song], for cacheKey: String) {
    guard openIfNeeded() else { return }
    guard let payload = try? encoder.encode(songs) else { return }

    let sql = """
      INSERT INTO song_cache(cache_key, payload, updated_at)
      VALUES(?, ?, ?)
